...
function getUrlFile()
{
    ...
    $path = isset($getUri[0]) ? $getUri[0] : ''; // php
    $action = isset($getUri[1]) ? $getUri[1] : ''; //server_apk_kasir
    $action2 = isset($getUri[2]) ? $getUri[2] : ''; //api
    $action3 = isset($getUri[3]) ? $getUri[3] : ''; // namaFile di routes

    if ('api' == $action2) {
        $file = 'src/routes/' . (isset($action3) ? $action3 : 'sites') . '.php';
        if (file_exists($file)) {
            return $file;
        }
    }
